"""Watchlist query and subscription diff for Polygon Options WS Ingestor.

NOTE: Watchlist is currently queried from Trade environment PG (per-env watchlist
table). This is a transitional approach — future iterations should use
platform-api watchlist union API to aggregate all Trade environments' watchlists
in a Plugin-native way.
"""
...
def _fetch_watchlist_sync(pg_params: dict) -> Set[str]:
    """Sync psycopg2 query; called via asyncio.to_thread."""
    import psycopg2
...
async def fetch_watchlist_symbols(pg_params: dict) -> Set[str]:
    """Async wrapper — runs psycopg2 query off the event loop."""
...
def channels_for_symbols(symbols: Set[str], tier: str, trades_enabled: bool) -> str:
    ...
    and trades (T.O) require developer tier.
    """
    if not symbols:
        return ""
