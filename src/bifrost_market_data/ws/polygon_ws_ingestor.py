"""Polygon Options WebSocket ingestor — Plugin-native implementation.

Writes option quotes to shared redis-massive bus (symmetric to IB Gateway → redis-ib).
"""
...
from bifrost_market_data.ws.redis_writer import PolygonRedisWriter
from bifrost_market_data.ws.retry import ReconnectPolicy
from bifrost_market_data.ws.subscription_manager import (
...
def _get_polygon_ws_cfg(config: dict) -> dict:
    """Extract Polygon WS settings from Plugin config."""
    import os

    pw = config.get("polygon_ws") or {}
    poly = config.get("polygon") or {}
    ...
    tier = (pw.get("tier") or poly.get("tier") or "starter").strip().lower()
    if tier not in ("starter", "developer"):
        tier = "starter"
    trades_enabled = bool(pw.get("trades_enabled", tier == "developer"))
    ws_enabled = massive_ws_enabled(tier, pw)
    ws_url = (pw.get("ws_url") or "wss://socket.polygon.io/options").strip()
...
class PolygonWsIngestor:
    """Polygon Options WS Ingestor — connects to Polygon WS, writes redis-massive."""

    def __init__(self, config: dict, *, redis_client: Any, pg_params: dict) -> None:
        self._cfg = config
        self._polygon = _get_polygon_ws_cfg(config)
        self._writer = PolygonRedisWriter(redis_client)
    ...
    async def run(self) -> None:
        ...
        logger.info(
            "Polygon WS ingestor starting (tier=%s, trades=%s, ws_enabled=%s)",
            self._polygon["tier"],
            self._polygon["trades_enabled"],
            self._polygon["ws_enabled"],
        )

        if not self._polygon["ws_enabled"]:
        ...
        logger.info(
            "Polygon WS ingestor stopped (msgs=%d, reconnects=%d)",
            self._msg_count,
            self._reconnects,
        )
    ...
    async def _run_rest_only_standby(self) -> None:
        ...
        logger.info(
            "Polygon WS standby (tier=%s): Options Starter uses REST aggregates, "
            "not live option WebSocket quotes. Set polygon_ws.ws_enabled=true after "
        ...
        logger.info("Polygon WS ingestor stopped (REST-only standby)")
    ...
    async def _run_session(self) -> None:
        ...
        try:
            async with websockets.connect(ws_url, ping_interval=None, ping_timeout=None) as _ws:
                await asyncio.wait_for(_ws.recv(), timeout=10)
                ...
                if isinstance(_resp, list):
                    ...
                    if "delayed" in _msgs:
                        ...
                        effective_url = ws_url_delayed
                        logger.info("Redirecting to delayed endpoint: %s", effective_url)
        except Exception as e:
            logger.warning("Auth probe failed: %s", e)
            return

        channels = channels_for_symbols(
            symbols, self._polygon["tier"], self._polygon["trades_enabled"]
        )
    ...
    async def _watchlist_refresh_loop(self, ws: Any, initial_symbols: Set[str]) -> None:
        ...
        while not self._stop.is_set():
            ...
            tier = self._polygon["tier"]
            trades = self._polygon["trades_enabled"]
