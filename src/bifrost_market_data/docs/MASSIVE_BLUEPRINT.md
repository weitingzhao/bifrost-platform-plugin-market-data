...
**持有我们付费买到的数据，深到计划允许的程度，新到本 session 要求的程度，中间没有洞，并且随时能证明这四件事——不需要任何人去跑一条查询。**

它不是一个"跑任务的东西"。任务是手段，持有的数据才是产物。这个区分不是措辞讲究：只要度量的单位是任务，一个每天报成功却什么都没采的 slot 就是不可见的。

## 2. 四个轴

一个数据集的健康只能用四个问题描述，缺一个都会漏掉一整类故障。**每个轴都必须有分母，分母是契约的一部分，不是面板临时决定的。**
...
| **厚度** | 中间有没有洞 | 该数据集声明的发布节奏（§4 C-C1） | 前三个轴全绿的一张筛子 |

四个轴互相独立：广度满而深度浅，是"刚开始采"；深度足而新鲜度差，是"停更了"；新鲜度好而广度窄，是"只盯着几个名字"；而三者全绿仍可能中间千疮百孔——**2026-09-09 实测，`stock_daily` 报 20,695 个标的、五年历史、当日新鲜，同时最近 90 天里有 7 个普通交易日只有 18 行**。厚度是被漏掉的那一维，它和新鲜度不是一回事：新鲜度问"今天的到了吗"，厚度问"过去那些天是不是都齐"。

**任何把四者压成一个"健康分"的做法都会把这四种完全不同的处境显示成同一个颜色。**
...
| 编号 | 契约 |
|---|---|
| **C-C1** | 每个数据集在契约表里声明**发布节奏**：`session`（每个交易日一批）或 `settlement`（按结算日等其他节律）。只有 `session` 的才拿交易日历判定缺失——`short_interest` 双月结算，按交易日衡量会报出 56 个从未欠下的缺口。**节奏是声明出来的，绝不能从被它评判的数据里反推。** |
| **C-C2** | **缺失日与稀薄日分开报，永不合并。** 一天没落地和一天落地了几乎是空的，对读的人都是一天数据没有，对修的人是两件事。 |
| **C-C3** | 稀薄的判定对**前序产出**，不对窗口中位数，也不对居中邻域。宇宙从 26 个名字扩到 575 是台阶不是洞：拿全窗口中位数比，台阶之前的每一天都成了洞；拿居中邻域比，台阶两侧仍然误判。洞低于它之前的产出，台阶上升和爬坡起点都不低于——而持续性的下跌仍然要报，一个腰斩后就没恢复的采集是退化，不是形状。 |
| **C-C4** | 厚度**在读取时计算，不靠事后记录**。记录只能看见从它打开那天起的事，而厚度的全部意义在于看见过去。一个逐 session 的检查（如 doctor）没有记忆：它当天报了 crit，那天过去洞就永久隐形。 |
...
| **C-G2** | 每个数据集都要能回答四个轴。缺一个轴是缺口，需要在校准里记录并给出最小改动，不能标成"不适用"。 |
| **C-G3** | **entitled 但未持有的能力必须可见**，并标注它能解锁什么下游能力。付了钱而没在采的数据面，如果不显示出来，就永远不会被决定。 |
| **C-G4** | **判定在插件里做，不在面板里做。** 阈值一旦只存在于某个前端，就没有第二个读者能看见它——doctor 用 Python 开处方，永远无法被告知一个判定退步了。面板只渲染插件给出的 verdict。 |
| **C-G5** | **时间会销毁的要向前记录，表里还在的要在读取时计算。** 判定除以「此刻有多晚」「当时的档位口径」，无法向后重算，所以每次 compute 的判定必须落库；厚度读的是仍然躺在表里的 session，所以不落库。两条规则不冲突，判据是**这个问题今天还能不能被重新问一遍**。 |
...
5. **中间有没有洞**（按各自的发布节奏，缺失日与稀薄日分列）
6. 没到的是谁的责任（vendor 无数据 / 未排程 / 采集失败 / 计划边界）

而且——**一个每天报成功却什么都没采的 slot，在这一屏上是红的；一个三个月前漏掉的那一天，今天仍然是红的。**
...
| 数据面 | 能解锁什么 |
|---|---|
| 新闻（`/v2/reference/news`） | 蓝图 §3.2 的事件面从"未测"变为可回测；Research 目前没有任何前瞻事件源。每篇带逐 ticker 的 `insights`（情绪 + 理由），是否作为叙事来源待 Owner 裁定（Vision §15 Q5） |
