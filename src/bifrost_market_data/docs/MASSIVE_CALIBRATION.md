...
2. **逐根提交**：修复改为拿 connection 而不是 cursor，每个 `(表, 根)` 提交一次。一条语句超时只损失那一个家族，而不是把整轮撤销。
...
### 订正：那 296 万行这个修复够不着（同版实测）

0.31.4 部署后 Job 一次干净通过、不再打印 `DDL failed`——但 `repaired` 是 **0**，不是「啃掉一块」。查下去：

```
错挂总行数          2,964,154
目录里还有该合约          135     ← 只有这些能被现在的 UPDATE 修
目录里已无          2,964,019     ← 99.995%
```

`UPDATE` 的 `EXISTS` 要求 `option_contract` 仍然收录那张合约。而 `option_contract` **最早的 expiry 是 2026-08-05**（`option_expiration` 同样），错挂的行几乎全是 2–7 月的 bar，对应合约早已到期并离开目录。

所以这个修复**不是「每次部署啃一块」，而是结构上够不着**：它能修的（那 89,790 行）早就修完了。逐根提交仍然值得——它拆掉的是「超时整体回滚 → 下一次报告『没写任何东西』」这个自我掩盖的形状——但它解决的是**回滚**，不是**积压**。**上面第 2 条原先的写法是错的，此处订正。**

**没有历史目录可用**，所以要动那 296 万行只剩两条路，都需要 Owner 拍板：

- **接受它**，记成一个有界的已知缺陷：受影响的是 35 个调整后根（`BDX1`/`APTV1`/`CDE2` 这类）在 2–7 月的历史 bar；下游 `WHERE underlying = 'BDX'` 查不到这段。新数据不再受影响（入口已修）。
- **放宽守卫**：对**根 = 规范符号 + 一位数字**的那 35 对（OCC 调整后合约的构造规则，一个合法股票代码不会长这样），不再逐合约向目录求证，直接按映射重写。风险从「逐行确认」降为「逐对确认」。第 36 对是 `BRKB → BRK.B`——标点差异不是调整后合约，形状不同，要单独判断。

在拍板之前不动那 296 万行：这是一次不可逆的批量 UPDATE。

### 再订正：判据换了，因为原来那条覆盖 0 行（0.31.6）

Owner 选了 B。按「根 = 规范符号 + 一位数字」实测，**符合的 26 个根在 `option_daily` 里一行都没有**。那 296 万行在完全不同的地方：

| root | → canonical | rows | 是什么 |
|---|---|---|---|
| `SPXW` | `SPX` | **2,799,555** | **SPX 周度期权的根**（月度 AM 交割用 `SPX`，周度/月末 PM 用 `SPXW`），标的都是 SPX |
| `BRKB` | `BRK.B` | 158,123 | OCC 去掉点号 |
| `ECHO` | `SATS` | 6,435 | 公司改名 |
| `VMRK` | `EQR` | 33 | 改名 |
| `IA` | `ISSC` | 8 | 改名 |

**一个调整后合约都没有。** 这个模块是为 OCC 数字后缀写的，而积压是三种别的现象穿着同一个症状。

判据因此换成两条**实测出来的**、互相独立的每对检查：

1. **目录把这个根唯一地映射到一个 underlying**——实测 `ECHO → {ECHO, SATS}`、`VMRK → {EQR, VMRK}` 两个根在目录里自己就有歧义，改名进行中，哪一张合约决定答案，只有逐合约的 `EXISTS` 知道；
2. **根本身不是一个活着的股票代码**——`ECHO` / `VMRK` / `IA` 三个都是，而一个指向活体的根正是「按映射重写会砸到真东西」的唯一场景。

`SPXW` 与 `BRKB` 两条都干净（各 27,340 / 2,082 张合约在目录里，都不是股票代码），合计 **2,957,678 行 = 99.8%**；`ECHO`/`VMRK`/`IA` 两条都不干净，**6,476 行保留守卫、原样不动**。

**这比原来批准的判据更严**：从「看起来像 OCC 调整根」换成「目录自己无歧义 且 不撞活体代码」，而且两条都是量出来的不是推出来的。

`SPXW` 一个根就是 280 万行，单条 UPDATE 在任何值得设的预算里都跑不完，所以按 `ctid` 分块，每块 5 万行、每块单独提交。一次部署 90 秒预算啃掉几块，剩下的下次继续——**这一次「每次部署啃一块」是真的**，因为块已经提交了。

## 2n. 一次我造成的数据损坏（0.31.6 → 0.31.7）

放宽守卫之后，`SPXW` 那 280 万行要分块重写。我写的分块 UPDATE 是：

```sql
UPDATE raw_market.option_daily d SET underlying = %s
WHERE d.ctid = ANY(ARRAY(SELECT ctid FROM ... WHERE underlying = %s LIMIT 50000))
```

外层 `WHERE` **只有 ctid**。而 `option_daily` **按 `bar_date` 分区，ctid 只在分区内唯一** —— 每个分区里同一个物理位置的行都被匹配并改写。
...
实测证据：`underlying='SPX'` 名下 618,118 行的 ticker 不是 `O:SPX*`（样例 `O:SPY250926P00485000`），`underlying='BRK.B'` 名下 866,869 行不是 `O:BRKB*`（样例 `O:AMD251017P00200000`）。**SPY 的期权写成了 SPX，AMD 的期权写成了 BRK.B。**

### 能复原，因为 ticker 没被动

`option_ticker` 一个字节都没改，而它编码着正确的根。所有被误改的行都来自「payload 不带 underlying」的回填 job，**它们损坏前的值恰好就是从 ticker 解析出的根**——算回去就是原值，不需要备份。

`restore_ctid_damage` 按 `underlying ∈ {SPX, BRK.B} 且 ticker 前缀不匹配` 定位，把 `underlying` 设回 ticker 自己的根。用 `NOT LIKE 'O:SPX%'` 而不是等值，所以被**正确**移动的 `O:SPXW…` 不会被倒回去。

复原条数 `{'SPX': 618118, 'BRK.B': 866869}` = **1,484,987**，与实测损坏数逐位相符；复原后两个标的的错配数都是 **0**。

### 两条留下来的东西

**谓词**：外层 `WHERE` 必须重复全部条件，ctid 列表只负责缩小批次。测试断言 `d.underlying` 出现在 `d.ctid` 之前——这个形状回不来了。

**顺序**：复原排在修复之前，一次部署不会先毁再毁。

### 我的失误

判据我用实测反复校正过三轮，**但那条 SQL 本身没验证就上了生产**。`option_daily` 是分区表这件事我当晚已经用过两次（`estimated_rows` 就是为它写的分区求和），却没有把它和 ctid 的语义连起来。

一次不可逆的 296 万行批量写入，**本该先在一个根上小批量试跑并核对条数**：Job 报 2,200,677 而应该是 598,438，这个差值在第一块就看得出来。「判据经过三轮实测」制造了一种安全感，而实际出事的是没被检查的那一部分。

## 2o. 三条「改问法」的声明，两条被数据否决（0.31.9）

我提出三条不花采集成本、只改判定口径的改动。先测后声明，结果**只有一条站得住**。

### ✅ treasury_yield：单序列也有深度

它 `symbol_column=None`——一张以 `yield_date` 为主键的单序列表。逐标的深度无标的可数，于是落进边界分支拿到空 population，轴报 `unknown`：**轴在说「我没看到」，而真相是「你拿逐标的的问题去问一个没有标的的东西」**。

实测跨度 **2021-09-01 → 2026-09-08，1,833 天**，目标 30 天。`_depth` 现在对「无 symbol 列 + rolling_days」判它的**序列跨度**，`single_series: true`，`1/1 达标` → **ok**。两个方向都可判：跨度不足同样会报 thin，不会退回 unknown。

### ❌ 「上市不足 N 年不参与深度判定」——没有数据可依

假设是：`stock_daily` 深度 3,370/5,316 的缺口是新上市。实测：

```
达标                3,361
未达标 · 无 list_date  1,955     ← 全部落在这一桶
未达标 · 上市不足 5 年     0
未达标 · 上市已满 5 年     0     （真缺口）
```

**`raw_market.ticker.list_date` 对全部 5,317 个活跃 ticker 都是空的。** 而 `ticker_sync.py:65` 明明在写这个字段——查下去是**端点不带**：

```
/v3/reference/tickers（列表）字段：active, cik, composite_figi, currency_name,
    last_updated_utc, locale, market, name, primary_exchange, share_class_figi,
    ticker, type          ← 没有 list_date
/v3/reference/tickers/A（详情）→ list_date: 1999-11-18
```

handler 没写错，它映射的是列表端点从不发送的字段。要让这条声明成立，**先得有 5,317 次逐票详情调用**（一次轮转，类似 `fundamentals-rotate`）——那是一件有成本的采集工作，不是一条声明。

### ❌ 「财报 83% 是 vendor 覆盖边界」——没有建立

假设是：缺财报的是 ETF、信托这类本来就没有财报的东西。实测**全部 901 个都是 `CS`（普通股）**，而有财报的 4,416 个也全是 `CS`——类型解释不成立。

退一步看年限：

```
无财报 901：满 5 年 331 · 2–5 年 202 · 1–2 年 147 · 不足 1 年 220
有财报 4,416：满 5 年 3,030 · 2–5 年 680 · 1–2 年 327 · 不足 1 年 379
```

缺财报的确实更年轻（24% 不足一年 vs 9%），**但其中 331 个有整整五年价格历史却一张财报都没有**。既不是「太新」也不是「不是股票」。最可能仍是 vendor 的财报宇宙小于 ticker 宇宙（与已记录的「ratios 覆盖约 5,000 个 ticker」同形），**但「最可能」不是声明一个 boundary 的依据**——把它标成边界等于断言一件我没有证实的事。保持 `partial`，并记下下一步是拿那 331 个里的几个去问 vendor 的 financials 端点。
...
### 全市场检查看不见完全缺席的 session（仪器的洞，不是数据的洞）
...
**修完第一次实测：500 天里 `absent_count = 0`。** 这个盲点今天没藏着东西——补的是仪器的缺口，不是数据的缺口。这一条照写，因为下一次它可能不是 0，而那时没人会想起来它曾经不会报。

### 一个我自己造出来的错误结论（同日撤回）

我先把 2025-06-02 … 06-20 那 14 天报成了「`stock_daily` 里没有任何标的的任何一行」，并据此说这条全市场检查「回答没有低覆盖日期」。**两句都是错的。**

真实读数（`days_back=500&min_symbols=100`）：

| 日期 | 该日 distinct symbol |
|---|---|
| 2025-05-30 | 11,090 |
| **2025-06-02 … 06-20（14 个交易日）** | **各 1** |
| 2025-06-23 起 | 恢复 ~11,000 |

所以那 14 天是**稀薄**，不是缺席——旧的 `HAVING` 查询本来就点名了它们。错误的来源是我读自己的探针输出时取了一个不存在的键（`dates`，而响应里是 `low_coverage_dates`），又没有打印 `count`，于是把「我没读到」当成了「它没报」。

**真正没人看见它的原因在 Console 不在插件**：`ReadinessPanel` 固定问 `days_back=30`（注释写明是为了躲开插件默认 420 在负载下的 500），屏幕上的窗口从来够不到 2025 年 6 月。洞一直被报着，只是没人被报到。

顺带一个未定性的读数：用契约的 session 门槛 12,000 去量，500 天里有 234 天在门槛之下，且 2025 年 6 月之前那一段稳定在 ~11,050。这**不一定**是缺陷——历史上市数不同、回填口径也可能窄于今天的全市场档。列在这里是为了下次有人量之前先知道它存在，不作为结论。

### 那 14 天原来一直被两道过滤挡着（0.38.2）

同日晚些时候把 Trade 交接的回填按钮接进 Ops 时才发现，那 14 天从来不是「没人报」，是**被报出来之后又被丢掉两次**：

1. **Console 的窗口是 30 天**（注释写明是为躲插件默认 420 在负载下的 500）。那 14 天在 ~480 天前，屏幕上的窗口从来够不到。实测同一个读数 30 天 0.6 秒、420 天 4.5 秒，都远在 60 秒网关之内——窗口已放宽到 500 天，且面板现在把窗口写进说明里（**不说出口的上限会被读成「再往前没有了」**）。
2. **放宽之后它们还是被丢**：`filterActionableLowCoverageDates` 以「`symbol_count < 500` 即非交易日」为判据，而这 14 天各 1 个标的。这条判据把两件相反的事混成一件——**休市日里漏进来几行是噪声，交易日只剩几行是最大的一种缺口**。

改法是让插件说清楚是哪一种：`low_coverage_dates` 每行新增 `session`（取自与缺席扫描同一份交易日历；日历读不到时是 `null`，因为放不进日历的日期是未知，不是「非交易日」）。Console 以它为准，阈值只作为没有该字段时的回退。
...
旧 SQL 原样在库里跑也是同一个错——vendor 的 short volume 带小数股，抽查的 6 个标的**自 2024-09-09 建表起每一行都是小数**，所以这条路由自有数据起就一直 500
...
- **P2（残缺 EOD 快照当天标出并补抓）**：已做，见下。
- **P5（`option_daily` / `short_volume` 保留上限）**：已做（0.41.0），见 §2u。
- **O2（CNPG 的 30 Gi）**：待 Owner 决定，未动。O1 已做（`bifrost-trade-infra` 侧）。

### 链到齐了，而且是错的（P2，0.40.0）

上面的覆盖检查回答「行来了没有」，这一条回答「来的是不是这一场的」。
Research 在 2026-09-22 那一场量到：12 个标的带 IV 的合约数只剩平常的一半左右，**IV 却高出 2–3 倍**，
前后两天都正常，同一天的大票也正常。AJG 当天用 Brent 从 `option_daily` 反解出 0.337，与前后一致 ——
**行情是对的，对行情的那一张快照不对。** NET 09-17、SHW 09-11 是同一个样子。

**两半都不能单用。** 当天挂牌少了，稀薄就是正常；IV 自己也会动。
但同一个名字、同一场、两件同时发生，不是市场会做的事。判据因此是合取：

```
带 IV 的合约数 < 前 5 场中位数的 60%   AND   IV 中位数 > 前 5 场中位数的 1.8 倍
```

与 research 0.118.0 拒绝投影的判据一致 —— 两边看同一件事，一边不投影，一边去补抓。

三条边界写进了检查而不是留给读者：

- **没有基线的名字不判。** 昨天才进 `research.option_universe` 的名字只有一场历史，拿它当基线会让每一次扩宇宙在第二天看起来都是坏的。
- **基线不足 20 个带 IV 的合约不判。** 6 的一半是 3，两个比值在这个量级上是舍入噪声。
- **问不出来就报 `unprobed`，不报干净。** 这是报告里最宽的一次读（6 场 × 全部可期权标的），所以给了自己的 25 秒预算；
  被取消的查询不是「读到了零」，此时既不清白任何名字也不指控任何名字。

**处方是定点补抓，不是重跑 slot。** `eod-pipeline` 的处方会把 662 个标的全部重新入队去修 12 个，
而当场修复的全部意义就在于它要塞进当天晚上。所以新增了 `enqueue` 的 `payloads` 形式：一条处方、一组 payload、每个名字一个 job。
两件事让它成立：去重是**只覆盖 pending/running 的部分索引**，所以原 job 已 `done` 时同样的 payload 会重新入队而不是被折叠；
handler 又是按 (option_ticker, 本场 anchor) upsert，所以坏行是被替换而不是被追加。
窗口取自 slot 自己那个函数，所以补抓买到的是**计划今天给这些名字的窗口**，不是写坏那些行时的窗口。

**时间窗是够的**：self-heal 00:45 UTC = 当地 20:45 NY，在 18:00 收集之后、下次开盘之前，`chain_session()` 还认同一场。
过了就不行 —— vendor 的链端点永远只返回当前这一场 —— 那时检查照报，但**不给处方**：
递一个会把今天的 greeks 写到过去那一场键下的调用，比沉默更糟。

**故意在 `EOD_CRITICAL_CHECKS` 之外**：扩大什么能拦住 Research 批次是关于闸门的决定，不是加一条检查的副作用。

**阈值是回测出来的，不是定出来的。** 发版后拿 `/market/options/snapshots` 把 Research 点名的两个名字逐场读回来，
用上线的那条规则重算 2026-09-22：

| 标的 | 09-22 | 前 5 场基线（中位） | 行数比 | IV 比 | 判定 |
|---|---|---|---|---|---|
| CDW | 27 行 · IV 1.213 | 52 行 · 0.467 | **0.52** | **2.60** | 命中 |
| AJG | 21 行 · IV 0.770 | 41 行 · 0.347 | **0.51** | **2.22** | 命中 |

前后两场全部干净（CDW 09-15..09-21 在 50–70 行、IV 0.45–0.50，09-23 回到 48 行 / 0.459）。
行数比的余量不宽（0.52 对 0.60），IV 比的余量很宽（2.2–2.6 对 1.8）——**这正是合取的意义**：
单看行数会在挂牌变动的日子误报，单看 IV 会在大波动的日子误报，两个一起才把这一天从旁边那几天里分出来。
同日全宇宙实测零误报（doctor 在 2026-09-24 上报 `No chain arrived thin and hot`）。

## 2u. 保留期不是一个新数字（P5，0.41.0）

库里最大的两张表没有上限：`option_daily` 9.65 GB / 3,800 万行，`short_volume` 4.7 GB / 700 万行（2026-09-25 实测）。
trim slot 原本只管 `job_ingest`（48 小时）、`option_snapshot`（90 场）、盘中（30 天）、`option_trades`（30 天）。

**显而易见的做法是在 trim 的配置里写一个天数，而那是错的。** 写在两处的窗口，就是一个深度轴承诺两年、trim 却留三年的数据集；
C-G1 之所以存在，正因为这个系统曾经对「应该有多少标的」有四个不同答案。

所以**上限就是契约已经声明的深度目标**（`contracts.retention_days`）：

| 数据集 | 深度目标 | 声明的理由 |
|---|---|---|
| `raw_market.option_daily` | 730 天 | Options Starter 滚动两年 |
| `raw_market.short_volume` | 730 天 | vendor 的 `?date` 大约供到两年前 |

一个数字。而且移动它会同时移动深度轴在量什么——这是对的，**更短的保留期确实就是更浅的数据集**。
Research 要两年是因为 History 页读 504 天、IV 百分位窗口在其中；vendor 供两年；计划的期权聚合滚动两年。三件事同一个数。

**名单是显式的，不是「所有声明了滚动窗口的数据集」。** `stock_daily` 也声明了五年（1825 天），
而它的窗口滚在 vendor 那一侧、不在这里——从它删行会把一条计划边界变成我们自己挖的洞，正是 C-D3 要防的那种归因错误。
往这个名单里加一张表是一个决定。`ratios`（forward_only）、`option_snapshot`（按场计）、`option_contract`（目录）都返回 `None`，
它们没有可按时钟过期的东西。

**两种机制，一条策略。** `option_daily` 已按月分区，所以先走 `drop_month_partitions_older_than`
（18 个分区全部 owner 是 `bifrost`，和 `option_snapshot` 那些归 `postgres` 的不同，所以掉得动）。

**但只掉分区是不够的，而"以为够"就是这一版差点犯的错。** `ensure_month_partitions` 只往回开 **12** 个月，
而契约要留 **24** 个月——所以比这更老的写入全部落进了 **DEFAULT 分区**，而掉分区的函数只匹配 `_yYYYYmMM$`，**永远碰不到它**：

```
option_daily_default    3680 MB   1,546 万行   2024-09-09 .. 2025-07-31
option_daily_y2025m08    426 MB   ← 月分区只从这里开始
```

**3.68 GB、全表的 41%、整个最老的一年。** 只掉分区的保留期会把它本该过期的那批数据一声不响地留下来。
所以行删除也要跑，而且按构造它只会碰 default 分区：其余每个月分区的区间都从 2025-08 起，本来就在窗口内。
default 也不会再收到新行（今天的 bar 有自己的分区），所以它会随着内容在未来十一个月里逐月出窗口而被磨掉。

`short_volume` 是单一无分区堆表，所以只能 DELETE，按主键分批提交。

两边用**同一个截断到月初的 cutoff**。这不是凑巧：实测 `short_volume` 最深 962 天、中位 746 天，
按整 730 天删会把量到的深度死死钉在目标上，让「达标」变成滚动窗口每晚都要跨过的一条刀刃。
截到月初意味着某一天只在它整个月都出了窗口之后才过期——**所以这里的保留期永远不会比契约更短，只会更整**。

**第一次跑会删掉什么（干读出来的，不是推算）**：cutoff 截到 2024-09-01。

| 表 | 首跑删掉 | 之后 |
|---|---|---|
| `option_daily` 月分区 | **0 个**（最老月分区 y2025m08 远在窗口内） | 2027-08 才会有第一个整月出窗口 |
| `option_daily` default | **0 行**（default 最老是 2024-09-09，仍在 cutoff 之后） | 2026-10-01 起每月约 150 万行，十一个月磨完 |
| `short_volume` | **220 行** | 进入密集段后每月约 40 万行 |

`short_volume` 那 220 行是这里唯一一处我先推算错、被干读纠正的地方：按 `oldest_days: 962` 和「每天约 13,000 行」
外推得到约 270 万行，而真实是 **220 行**——`oldest_days` 是**跨标的的最大值**，不是均匀深度，
密集段其实从 2024-09 才开始（中位 746 天正好对上）。**深度轴的一个极值不能当分布用。**
...
| 快照 | 日期 | 说明 |
|---|---|---|
| 2026-09-25.7 | 2026-09-25 | §2u 更正（0.41.1，未部署前抓到）：`option_daily` **只掉分区是不够的**。`ensure_month_partitions` 只往回开 12 个月而契约留 24，所以更老的写入全在 **DEFAULT 分区**——实测 `option_daily_default` 1,546 万行 / 3.68 GB / 2024-09-09..2025-07-31，全表的 41%、整个最老的一年，而掉分区的函数只匹配 `_yYYYYmMM$` 永远碰不到它。已补上行删除（按构造只碰 default）。另更正一处推算：`short_volume` 首跑实际只删 **220 行**，不是外推的 270 万——`oldest_days` 是跨标的最大值不是均匀深度。|
| 2026-09-25.6 | 2026-09-25 | §2u：P5 落地（0.41.0）。`option_daily` 与 `short_volume` 有了保留期，而**上限不是一个新数字**——就是契约已声明的深度目标（`contracts.retention_days`，两张都是 730 天）。名单显式：`stock_daily` 也声明五年但它的窗口滚在 vendor 那侧，从它删行等于自己挖洞。两种机制一条策略：`option_daily` 只掉分区不删行（3,800 万行上 DELETE 收不回空间，只加死元组），`short_volume` 无分区只能分批 DELETE，两边共用截断到月初的 cutoff——实测它最深 962 天中位 746 天，按整 730 删会把深度钉在目标上变成刀刃。第一次跑：option_daily 掉 0 个分区（最老分区正在边界上，首掉是 10-01），short_volume 删约 208 天。|
| 2026-09-25.5 | 2026-09-25 | §2t：P2 的阈值回测。用上线的规则重算 2026-09-22——CDW 行数比 0.52 / IV 比 2.60，AJG 0.51 / 2.22，两个都命中；前后两场全干净；09-24 全宇宙零误报。行数比余量不宽而 IV 比余量很宽，合取是必需的。|
| 2026-09-25.4 | 2026-09-25 | §2t 续：P2 落地（0.40.0）。doctor 新增 `option_snapshot_degraded`——带 IV 的合约数 < 前 5 场中位数 60% **且** IV 中位数 > 1.8 倍才判，与 research 0.118.0 同一判据。没有基线的名字不判、基线不足 20 个合约不判、问不出来报 `unprobed` 不报干净。处方是定点补抓（`enqueue` 新增 `payloads`，一条处方一组 payload），不是重跑 662 个标的的 slot；去重是只覆盖 pending/running 的部分索引，所以原 job 已 done 时同一 payload 会重新入队。链已翻场时照报但不给处方。故意在 `EOD_CRITICAL_CHECKS` 之外。|
