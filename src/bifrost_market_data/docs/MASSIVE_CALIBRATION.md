...
**两种机制，一条策略。** `option_daily` 已按月分区，所以走 `drop_month_partitions_older_than`，
**并且不给行删除兜底**——这一点和上面的 `option_snapshot` 不同，是有意的：在 3,800 万行上 DELETE 一次什么空间都收不回来，
只会给这一轮正在别处压缩的那张表再加 9 GB 死元组，而 DROP 是把空间还给文件系统。
...
**第一次跑会删掉什么（按实测推算）**：`option_daily` 最老的一根 bar 在 746 天前（y2024m09 分区），
cutoff 截到 2024-09-01，没有整月早于它，**所以今晚一个分区都不掉**；第一次真掉是 2026-10-01（那时 cutoff 变成 2024-10-01），
掉完剩下的最老日期正好是 730 天前。`short_volume` 最深 962 天，所以第一次会删掉约 208 天的行——
批量提交、每晚 60 秒预算，所以会分几晚排空而不是一次长事务。
